...
def main():
    ...
    for image_id, filename in enumerate(label_files_chck):
        ...
        out_img_file = osp.join(args.output_dir, "JPEGImages", img_name)  # png, jpg 변환하지 않고 확장자 상관없이 원본으로 저장
        ...
        # origin_img = Image.open(os.path.join(args.input_dir, img_name))          ############### pillow를 쓰면 가로 세로가 변경되는 이미지가 존재함!
